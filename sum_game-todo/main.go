...
For Bob to win, the sum of the digits in the first half of num must be equal to
the sum of the digits in the second half. For Alice to win, the sums must not be
equal.
...
func sumGame(num string) bool {
	lSum, rSum := 0, 0
	lHalf, rHalf := num[:len(num)/2], num[len(num)/2:]
