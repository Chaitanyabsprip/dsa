...
cells can flow to the Pacific and Atlantic oceans, as shown below: [0,4]: [0,4]
-> Pacific Ocean [0,4] -> Atlantic Ocean [1,3]: [1,3] -> [0,3] -> Pacific Ocean
[1,3] -> [1,4] -> Atlantic Ocean [1,4]: [1,4] -> [1,3] -> [0,3] -> Pacific Ocean
[1,4] -> Atlantic Ocean [2,2]: [2,2] -> [1,2] -> [0,2] -> Pacific Ocean [2,2] ->
[2,3] -> [2,4] -> Atlantic Ocean [3,0]: [3,0] -> Pacific Ocean [3,0] -> [4,0] ->
Atlantic Ocean [3,1]: [3,1] -> [3,0] -> Pacific Ocean [3,1] -> [4,1] -> Atlantic
Ocean [4,0]: [4,0] -> Pacific Ocean [4,0] -> Atlantic Ocean Note that there are
other possible paths for these cells to flow to the Pacific and Atlantic oceans.

Example 2:

Input: heights = [[1]] Output: [[0,0]] Explanation: The water can flow from the
only cell to the Pacific and Atlantic oceans.
...
[[false false false false false true] [false false false false false true] [false false false false false true] [false false false false false true] [false false false false false true] [true true true true true true]]
...
func pacificAtlantic(heights [][]int) [][]int {
	...
	for i := 0; i < len(heights); i++ {
		recurse(i, 0, &pacific, heights)
		recurse(i, len(heights[0])-1, &atlantic, heights)
	}
	for i := 0; i < len(heights[0]); i++ {
		recurse(0, i, &pacific, heights)
		recurse(len(heights)-1, i, &atlantic, heights)
	}
	result := make([][]int, 0)
	for i := 0; i < len(heights); i++ {
